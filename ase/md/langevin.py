...
class Langevin(MolecularDynamics):
    ...
    def step(self, f):
        ...
        if self.fixcm:
            ...
            m = atoms.get_masses()
            old_pcom = np.dot(m, old_moms / m.sum())
        ...
        if self.fixcm:
            new_p = V*self.masses
            new_pcom = np.dot(m, new_p / m.sum())
            pd = old_pcom - new_pcom
            pcorr = (np.tile(pd,(natoms,1)).T/m).T
            V = (new_p + pcorr) / self.masses
            print V*self.masses
