...
class Siesta_Raman(Vibrations):
    ...
    *get_frequencies()*, *get_spectrum()* and *write_spectra()*
    ...
    """
    def __init__(self, atoms, siesta, mbpt_inp, indices=None, name='ram', delta=0.01,
    ...
    def read(self, method='standard', direction='central'):
        ...
        [forces_zero, dipole_zero, freq_zero, pol_zero] = pickle.load(open(name))
        ...
        for a in self.indices:
            for i in 'xyz':
                ...
                [fminus, dminus, frminus, pminus] = pickle.load(open(name + '-.pckl'))
                [fplus, dplus, frplus, pplus] = pickle.load(open(name + '+.pckl'))
                if self.nfree == 4:
                    [fminusminus, dminusminus, frminusminus, pminusminus] = pickle.load(
                        open(name + '--.pckl'))
                    [fplusplus, dplusplus, frplusplus, pplusplus] = pickle.load(
                        open(name + '++.pckl'))
        ...
        # Raman
        dadq = np.array([(dadx[j, :, :, :]/(units.Bohr**2)) / sqrt(m[self.indices[j / 3]] *\
                units._amu / units._me) for j in range(ndof)])
        ...
        gk2 = ((dadQ[:, 0, 0, :] - dadQ[:, 1, 1, :])**2 + (dadQ[:, 1, 1, :] - dadQ[:, 2, 2, :])**2 +
            (dadQ[:, 2, 2, :] - dadQ[:, 0, 0, :])**2 + 6*(dadQ[:, 0, 1, :]**2 + dadQ[:, 1, 2, :]**2 
            + dadQ[:, 2, 0, :]**2))
    ...
    def get_pol_freq(self, freq, freq_array, quantity):
        ...
        w, interpol = interpolate(freq_array, quantity, 100*quantity.shape[0])
        i = 0
        while w[i] < freq:
            i = i+1
    ...
    def summary(self, method='standard', direction='central', 
        ...
        parprint('  #    meV     cm^-1   ' + iu_string_ir + '       ' + iu_string_ram +  '                  '+ iu_string_ram +  '                   ' + iu_string_ram, file=log)
        parprint('---------------------------------------------------------------------------------------------------------------------------', file=log)
        for n, e in enumerate(hnu):
            if e.imag != 0:
                ...
                arr.append([n, 1000*e, s*e, iu_ir * self.intensities_ir[n], 
                  iu_ram*self.intensities_ram[n].real, iu_ram*self.intensities_ram[n].imag])
            parprint(('%3d %6.1f%s  %7.1f%s  ' + iu_format_ir + iu_format_ram +  iu_format_ram + iu_format_ram) % 
                     (n, 1000 * e, c, s * e, c, iu_ir * self.intensities_ir[n], 
                     iu_ram*self.intensities_ram[n].real, iu_ram*self.intensities_ram[n].imag, iu_ram*self.intensities_ram_enh[n].real),
                     file=log)
        parprint('-----------------------------------------------------------------------------------------', file=log)
    ...
    def write_latex_array(self, fname = 'vib_latex_array.tex.table', caption='', nb_column = 5,
        hline = False, method='standard', direction='central', intensity_unit_ir='(D/A)2/amu', 
        intensity_unit_ram='au', label = 'tab_vib', log=stdout):
        """
        ...
        f.write('     Mode & Frequency (meV) & Frequency ($cm^{-1}$) & Intensity IR  (' + iu_string_ir + 
                    ')  & Intensity Raman (' + iu_string_ram + ') \n')
        if hline:
          f.write("   \hline \n")
        for n, e in enumerate(hnu):
            if e.imag != 0:
                ...
                c = ' '
                e = e.real
            f.write(('     %3d & %6.1f & %s  & %7.1f & %s  & ' + iu_format_ir + '  & ' + iu_format_ram + ' \n') % 
                    (n, 1000 * e, c, s * e, c, iu_ir * self.intensities_ir[n], iu_ram*self.intensities_ram[n].real))
            if hline:
              f.write(r"      \hline \n")
        f.write("    \end{tabular} \n")
        f.write("  \end{center} \n")
    ...
    def write_spectra(self, out='ram-spectra.dat', start=800, end=4000, 
                      npts=None, width=10, 
                      type='Gaussian', method='standard', direction='central', 
                      intensity_unit_ir='(D/A)2/amu', intensity_unit_ram='au', normalize=False):
        ...
        in cm^-1."""
        energies, spectrum_ir, spectrum_ram, spectrum_ram_enh = self.get_spectrum(start, end, npts, width, 
                                               type, method, direction, normalize)
        ...
        for row in outdata:
            fd.write('%.3f  %15.5e  %15.5e %15.5e  %15.5e   %15.5e    %15.5e\n' % 
                     (row[0], iu_ir * row[1], row[2], iu_ram * row[3], row[4], iu_ram * row[5], row[6]))
        fd.close()
