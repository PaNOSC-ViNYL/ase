...
class Render(gtk.Window):
    ...
    def __init__(self, gui):
        ...
        self.set_title('Render current view in povray ... ')
        vbox = gtk.VBox()
        vbox.set_border_width(5)
        self.natoms = self.gui.images.natoms
        pack(vbox, [gtk.Label("Rendering "+str(self.natoms) + " atoms.")])
        ...
        self.render_constraints = gtk.CheckButton("Render constraints")
        ...
        pack(vbox, [gtk.Label("Width"), self.width,
                    gtk.Label("     Height"), self.height,
        ...
        self.render_cell = gtk.CheckButton("Render unit cell")
        ...
        pack(vbox, [gtk.Label("Line width"),
                    self.line_width,
                    gtk.Label("Angstrom           "),
                    self.render_cell])
        pack(vbox, [gtk.Label("")])
        ...
        set_name = gtk.Button("Set")
        set_name.connect("clicked",self.set_outputname,"")
        pack(vbox,[gtk.Label("Output basename: "), self.basename,set_name])
        self.outputname = gtk.Label("")
        pack(vbox,[gtk.Label("               Filename: "),self.outputname])
        pack(vbox,[gtk.Label("")])
        self.tbox = gtk.VBox()
        ...
        pack(vbox,[gtk.Label("Camera type: "),self.camera_style,
                   gtk.Label("     Camera distance"),self.camera_distance])
        self.single_frame = gtk.RadioButton(None,"Render current frame")
        self.nimages = self.gui.images.nimages
        self.iframe = self.gui.frame
        self.movie = gtk.RadioButton(self.single_frame,"Render all "+
                                     str(self.nimages)+" frames")
        ...
        self.keep_files = gtk.CheckButton("Keep povray files       ")
        self.keep_files.set_active(False)
        self.keep_files_status = True
        self.window_open = gtk.CheckButton("Show output window")
