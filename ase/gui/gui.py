...
class GUI(View, Status):
    ...
    def get_menu_data(self, show_unit_cell, show_bonds):
        ...
        return [
            (_('File'),
             [M(_('Open'), self.open, 'Ctrl+O'),
              M(_('New'), self.new, 'Ctrl+N'),
              M(_('Save'), self.save, 'Ctrl+S'),
              M('---'),
              M(_('Quit'), self.exit, 'Ctrl+Q')]),

            (_('Edit'),
             [M(_('Select all'), self.select_all),
              M(_('Invert selection'), self.invert_selection),
              M(_('Select constrained atoms'), self.select_constrained_atoms),
              M(_('Select immobile atoms'), self.select_immobile_atoms,
                key='Ctrl+I'),
              M('---'),
              M(_('Copy'), self.copy_atoms, 'Ctrl+C'),
              M(_('Paste'), self.paste_atoms, 'Ctrl+V'),
              M('---'),
              M(_('Hide selected atoms'), self.hide_selected),
              M(_('Show selected atoms'), self.show_selected),
              M('---'),
              M(_('Modify'), self.modify_atoms, 'Ctrl+Y'),
              M(_('Add atoms'), self.add_atoms, 'Ctrl+A'),
              M(_('Delete selected atoms'), self.delete_selected_atoms,
                'Backspace'),
              M('---'),
              M(_('First image'), self.step, 'Home'),
              M(_('Previous image'), self.step, 'Page-Up'),
              M(_('Next image'), self.step, 'Page-Down'),
              M(_('Last image'), self.step, 'End')]),

            (_('View'),
             [M(_('Show unit cell'), self.toggle_show_unit_cell, 'Ctrl+U',
                value=show_unit_cell > 0),
              M(_('Show axes'), self.toggle_show_axes, value=True),
              M(_('Show bonds'), self.toggle_show_bonds, 'Ctrl+B',
                value=show_bonds),
              M(_('Show velocities'), self.toggle_show_velocities, 'Ctrl+G',
                value=False),
              M(_('Show forces'), self.toggle_show_forces, 'Ctrl+F',
                value=False),
              M(_('Show Labels'), self.show_labels,
                choices=[_('None'),
                         _('Atom Index'),
                         _('Magnetic Moments'),
                         _('Element Symbol')]),
            ...
            (_('Tools'),
             [M(_('Graphs ...'), self.plot_graphs),
              ...
              M(_('Move atoms'), self.toggle_move_mode, 'Ctrl+M', False),
              M(_('Rotate atoms'), self.toggle_rotate_mode, 'Ctrl+R', False),
              M(_('Orient atoms'), self.toggle_orient_mode, 'Ctrl+T', False),
              M(_('NEB'), self.neb),
              M(_('Bulk Modulus'), self.bulk_modulus)]),
            ...
            (_('Setup'),
             [M(_('Bulk Crystal'),
              ...
              M(_('Nanotube'), self.nanotube_window),
              M(_('Graphene'),
