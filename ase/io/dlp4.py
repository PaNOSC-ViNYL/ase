...
def read_dlp4(f):
   """ read a DL_POLY_4 config/revcon file
       usage read("name", format = 'dlp4')
       can be unforgiven with custom chemical element names
       please complain to alin@elena.space for"""
   line = f.readline()
     ...
     for j in range(3):
       ...
       for i in range(3):
         try:
           ...
           raise RuntimeError("error reading celll")
   symbols = []
   positions = []
   velocities = []
   forces = []
...
def write_dlp4(f, atoms, levcfg = 0, title = 'CONFIG generated by ASE'):
    """ writes a DL_POLY_4 config file
       usage write("name", atoms, format = 'dlp4')
       can be unforgiven with custom chemical element names
       please complain to alin@elena.space for"""
