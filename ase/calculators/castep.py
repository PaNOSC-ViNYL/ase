        ...
        while True:
            try:
                ...
                if not line or out.tell() > record_end:
                    ...
                    while True:
                        ...
                        if len(fields) == 6:
                            break
                    for i in range(3):
                        lattice_real.append(list(map(float, fields[0:3])))
                        lattice_reci.append(list(map(float, fields[3:7])))
                        line = out.readline()
                        fields = line.split()
                    ...
                    for n in range(n_atoms):
                        ...
                        positions_frac.append(list(map(float, fields[3:6])))
                        line = out.readline()
                        fields = line.split()
                ...
                # the CASTEP output)
                    ...
                    for n in range(3):
#                        print line
                        stress.append(list(map(float, fields[2:5])))
                        line = out.readline()
                        fields = line.split()
        ...
        if self.atoms and not self._set_atoms:
            ...
            if not all(atoms_assigned):
                not_assigned = [i for (i, assigned)
                                in zip(list(range(len(atoms_assigned))),
        ...
        while True:
            ...
            if "Number of symmetry operations" in line:
                ...
                for _ in range(nsym):
                    ...
                    for _ in range(3):
                        line = f.readline()
                        rotation.append(list(map(float, line.split()[1:4])))
                    while True:
                        if "displacement" in f.readline():
                            break
                    line = f.readline()
                    displacement = list(map(float, line.split()[1:4]))
