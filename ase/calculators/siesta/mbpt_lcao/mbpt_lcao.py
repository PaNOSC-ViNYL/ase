...
class MBPT_LCAO:
  ...
  Take into the kwargs arguemnt the input for the program,
  see ase/ase/test/siesta/test_scripts/Na8/script_mbpt_lcao.py for a complete
  example.
  """
  ...
  def write_tddft_inp(self):
    ...
    for k, v in self.param.items():
      if type(v) == type(np.array([0.5, 0.5, 0.5])):
        f.write(k + '  {0}  {1}  {2}\n'.format(v[0], v[1], v[2]))
  ...
  def run_mbpt_lcao(self, output_name='mbpt_lcao.out', write_inp=False):
    ...
    INPUT ARGUMENTS:
    ----------------
      output_name (optional, string): name of the output file, defualt: mbpt_lcao.out
      write_inp (bool): write the tddft_lr,inp file before to run the program, by default False
    """
    import subprocess
