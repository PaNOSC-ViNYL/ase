...
class read_mbpt_lcao_output:
  """
  Top class of the module: the one to be call by external function
  """
  def __init__(self):
    """
      the args and prop argument need to be changed as function of what you wish to plot
    """
  ...
  def Read(self, YFname=None):
    if self.args.folder != './' and self.prop.fatoms == 'domiprod_atom2coord.xyz':
...
class read_hdf5_data:
  ...
  def __init__(self, args, prop_fig, YFname):
    ...
    if args.tem_iter == 'iter':
      ...
      raise ValueError('only tem and iter!!')
  ...
  def set_carac(self, args_p, prop):
    ...
    for i in range(len(self.pl_num)):
      if self.pl_num[i] == '.':
        self.pl_file = self.pl_num[0:i] + '-' + self.pl_num[i+1:len(self.pl_num)]
  ...
  def determine_fname(self, args_p, perso=None):
    ...
    if perso == None:
      self.fname = args_p.folder + 'tddft_' + args_p.tem_iter + '_output.hdf5'
    else:
  ...
  def check_file_tem(self, args_p):
    ...
    if args_p.quantity == 'spectrum':
      ...
      if args_p.time == 0:
        ...
        if args_p.tem_input['tem_method'] == 'C':
          quantity = 'field_spatial_cc_freq_{0:.2f}'.format(args_p.plot_freq)
        else:
          quantity = 'field_spatial_freq_{0:.2f}'.format(args_p.plot_freq)
      else:
        quantity = 'field_time'

    print('quantity: ', quantity)
    chain = '_v{0:.8f}_bx{1:.8f}_by{2:.8f}_bz{3:.8f}'.format(args_p.tem_input['vnorm'],\
              args_p.tem_input['b'][0], args_p.tem_input['b'][1], args_p.tem_input['b'][2])
    if quantity == None:
      ...
      elif args_p.quantity in field_spatial or quantity == 'field_time':
        ...
        if args_p.quantity in field_spatial:
          ...
          if args_p.quantity == 'intensity':
            for i in range(args_p.time_num):
              self.dname.append('data_time_{0}/'.format(i) + args_p.quantity)
          else:
            for i in range(args_p.time_num):
              self.dname.append('data_time_{0}/'.format(i) + args_p.quantity + '_' + args_p.ReIm)

    if type(self.dname) == type([]):
      for i, name in enumerate(self.dname):
        print(name)
        sub_group = self.group['data_time_{0}/'.format(i)]
        if args_p.quantity not in sub_group.keys():
          raise ValueError(name + ' not saved in ' + self.fname + '. check with h5ls command.')
    else:
      if self.dname not in self.group.keys():
        print(self.group.keys())
        raise ValueError(self.dname + ' not saved in ' + self.fname + '. check with h5ls command.')
    ...
    if args_p.quantity == 'spectrum':
      ...
      if args_p.time == 0:
        ...
        if args_p.quantity == 'Efield':
          self.Array = dico['Array'].ravel('F').reshape(dim[0], dim[1], dim[2], dim[3])
        else:
        ...
        if args_p.quantity == 'Efield':
          self.Array = np.array((self.t.shape[0], sh[0], sh[1], sh[2], sh[3]), dtype=float)
          for i in range(len(self.dname)):
            self.Array[i, :, :, :, :] = self.group[self.dname[i]].value
        else:
          self.Array = np.zeros((self.t.shape[0], sh[0], sh[1], sh[2]), dtype=float)
          for i in range(len(self.dname)):
            print('array ', self.Array.shape)
            print('sum(data) ', np.sum(abs(self.group[self.dname[i]].value)))
            self.Array[i, :, :, :] = self.group[self.dname[i]].value
  ...
  def check_file_iter(self, args_p):
    if args_p.quantity == 'polarizability':
      ...
      quantity = 'field_spatial_freq_{0:.2f}_'.format(args_p.plot_freq) + args_p.inter

    if quantity not in self.File.keys():
      raise ValueError(quantity + ' not saved in ' + self.fname + '. check with h5ls command.')
    ...
    if self.dname == 'intensity':
      ...
      if args_p.species != '':
        if self.dname+'_'+args_p.ReIm + '_' + args_p.species not in self.group.keys():
          raise ValueError(self.dname+'_'+args_p.ReIm + '_' + args_p.species + ' not saved in ' + self.fname + '. check with h5ls command.')
      else:
        if self.dname+'_'+args_p.ReIm not in self.group.keys():
          raise ValueError(self.dname+'_'+args_p.ReIm + ' not saved in ' + self.fname + '. check with h5ls command.')
  ...
  def extract_data_iter(self, args_p):
    if args_p.quantity == 'polarizability':
      ...
      if args_p.species != '':
        self.Array = self.group[self.dname + '_' + args_p.ReIm + '_' + args_p.species].value.T
      else:
      ...
      if self.dname == 'dens':
        keys_f = ['dr', 'origin_dens', 'ibox_dens', self.dname + '_' + args_p.ReIm]
      elif self.dname == 'intensity':
        keys_f = ['dr', 'origin', 'ibox', self.dname]
      else:
        keys_f = ['dr', 'origin', 'ibox', self.dname + '_' + args_p.ReIm]
  ...
  def mesh2D(self):
    ...
    for j in range(self.xy_mesh.shape[1]):
      for i in range(self.xy_mesh.shape[0]):
        self.xy_mesh[i, j] = self.box[0][1] + i*self.dr[1]+self.origin[1]

    for i in range(self.xz_mesh.shape[0]):
      for j in range(self.xz_mesh.shape[1]):
        self.xz_mesh[i, j] = self.box[0][2] + j*self.dr[2]+self.origin[2]
    ...
    for j in range(self.yx_mesh.shape[1]):
      for i in range(self.yx_mesh.shape[0]):
        self.yx_mesh[i, j] = self.box[0][0] + i*self.dr[0]+self.origin[0]

    for i in range(self.yz_mesh.shape[0]):
      for j in range(self.yz_mesh.shape[1]):
        self.yz_mesh[i, j] = self.box[0][2] + j*self.dr[2]+self.origin[2]
    ...
    for j in range(self.zx_mesh.shape[1]):
      for i in range(self.zx_mesh.shape[0]):
        self.zx_mesh[i, j] = self.box[0][0] + i*self.dr[0]+self.origin[0]

    for i in range(self.zy_mesh.shape[0]):
      for j in range(self.zy_mesh.shape[1]):
        self.zy_mesh[i, j] = self.box[0][1] + j*self.dr[1]+self.origin[1]

  #
  #
  #
  def mesh3D(self):
    self.xmesh = np.zeros((self.dim[0], self.dim[1], self.dim[2]), dtype=float)
...
class read_text_data:
  """
  Class that read the output data of the tddft program for the field enhancement.
  can read .dat files (text file), .npy files (binary files) or .hdf5 files (binary).
  ...
  -----------------
    ...
    kwargs (optionnal):
      new_data: (list of args class!), list of the other args if needed to get more than one data
  Output parameters:
  ------------------
  ...
  def __init__(self, args, prop_fig, YFname):
    ...
    self.dr, self.origin, self.lbound, self.ubound, self.Array, self.box, self.dim = self.read_txt(args, self.fname)
  ...
  def determine_fname(self, args_p, perso=None):
    ...
    if perso == None:
      if args_p.quantity == 'Efield':
        fname = [args_p.folder + 'e_field_' + args_p.ReIm + '.x_' + args_p.inter +args_p.tem_iter + '.dat',\
                args_p.folder + 'e_field_' + args_p.ReIm + '.y_' + args_p.inter +args_p.tem_iter + '.dat',\
                args_p.folder + 'e_field_' + args_p.ReIm + '.z_' + args_p.inter +args_p.tem_iter + '.dat']
      elif args_p.quantity == 'intensity':
        fname = args_p.folder + args_p.quantity + '_' + args_p.inter + '_' + args_p.tem_iter + '.dat'
      elif args_p.quantity == 'polarizability':
        fname = args_p.folder + 'dipol_' + args_p.inter + '_' + args_p.tem_iter + '_krylov_' + args_p.ReIm + '.txt'
      else:
        fname = args_p.folder + args_p.quantity + '_' + args_p.ReIm + '_' + args_p.inter + '_' + args_p.tem_iter + '.dat'
    else:
  ...
  def read_txt(self, args_p, fname):
    ...
    if args_p.quantity == 'Efield':
      ...
      Array = np.zeros((A[0].shape[0], A[0].shape[1], A[0].shape[2], 3), dtype = float)
      Array[:, :, :, 0] = A[0]
      Array[:, :, :, 1] = A[1]
      Array[:, :, :, 2] = A[2]
      ...
      Array = np.loadtxt(fname)[:, 2:11].reshape(self.freq.shape[0], 3, 3)
      dr = 0.0
      origin = 0.0
      lbound = 0.0
      ubound = 0.0
...
class MBPT_LCAO_Parameters:
  ...
  The parameters are:
    self.quantity (string, default:'polarizability'): the type of data than one wish to plot, can be
                                      intensity
                                      efield
                                      potential
                                      dens
    ...
    self.format_input (string, default:'.hdf5'): format of the input file, can be
                                                ...
                                                .npy
                                                .hdf5
    self.folder (string, default: './'): name of the folder where are save the input data
    ...
    self.movie (int, default: 0) : if tem and plot_dens_time=1 (in tddft_lr.inp) then do a movie
  """
  def __init__(self):
    ...
    self.movie = 0
    self.plot_freq = 0.0
    self.tem_input = {'dr': np.array([0.3, 0.3, 0.3]), 'vnorm': 1.0, 'v': np.array([1.0, 0.0, 0.0]), 
                      'dw': 0.1, 'b': np.array([0.0, 0.0, 0.0]), 'vrange': None, 'brange': None,
                      'tem_method': 'N'}
    self.exportData = {'export': False, 'dtype': 'HDF5', 'fname': 'exportData'}
  ...
  def check_input(self):
    ...
    """
    self.param = {'quantity': [self.quantity, type('temoin')],
             'inter': [self.inter, type('temoin')],
             'ReIm': [self.ReIm, type('temoin')],
             'format_inp': [self.format_input, type('temoin')],
             'folder_inp': [self.folder, type('temoin')],
             'time': [self.time, type(0)],
             'time_num': [self.time_num, type(0)],
             'tem_iter': [self.tem_iter, type('temoin')],
             'species': [self.species, type('temoin')],
             'movie': [self.movie, type(0)],
             'tem_input': [self.tem_input, type({'y': 0})],
             'exportData': [self.exportData, type({'y': 0})]
      }
    ...
    for keys, values in self.param.items():
      if type(values[0]) != values[1]:
        raise ValueError('Error: input ' + keys + ' not right type, must be ' + str(values[1]))

      if keys in fields.keys():
        if values[0] not in fields[keys]:
          raise ValueError(keys + ' can be only: ' + str(fields[keys]))
        
        if (values[0] == 'spectrum' or values[0] == 'E_loss') and self.tem_iter != 'tem':
...
class MBPT_LCAO_Properties_figure:
  ...
  -----------
    ...
    self.output (string, default: 'pdf'): format of the output file, can be
                                        ...
                                        eps
                                        svg
    self.animation (int, default: 0): save a movie of the animation made by self.dynamic
                                      only available with Mayavi plot and if self.dynamic=1
    self.Edir (int, default: 0): plot direction of the E field, 0 or 1
    self.coord_Ef (2D numpy array default:'default') plotting coordinate E field??
    self.plot (string, default:'2D'): define the plotting method, can be
                                      1D
                                      2D
                                      3D
                                      Mayavi
    self.coord (1D numpy array, default: np.array([0.0, 0.0, 0.0])) coordinate for curve in 1D
  ...
  def __init__(self):
    ...
    self.cmap = cm.jet
    self.units = 'au' #for the moment only the polarizability can be modify in nm**2
    self.vmin = None
    self.vmax = None
    self.mayavi_screenshot = 0 #to save a mayavi scrennshot in order to perform subplot with matplotlib

    self.maya_prop = {'extent_factor': 1.0, 'figsize': (640, 480), 'contours': 3, 'atoms_resolution': 8,\
