...
class EMT:
    data = {'Cu': ('fcc', 3.60, 3.3, 11.0, 11.0),
            'H': ('dimer', 0.73, 4.5, 1.0, 1.0),
            'C': ('diamond', 2.9, 7.3, 1.0, 1.0)}
    ...
    def initialize(self, atoms):
        ...
        for Z in self.numbers:
            if Z not in self.par:
                structure, a, e, n, v = self.data[chemical_symbols[Z]]
                if structure == 'fcc':
                    N1 = 12
                    d1 = a / sqrt(2)
                    N2 = 6
                ...
                A = (N1 + N2 * (d2 / d1)**2) * n
                B = -2 * rc * (N1 + N2 * d2 / d1) * n
                C = (N1 + N2) * rc**2 * n
                E0, E1, E2, E3 = npy.linalg.solve(
                    npy.array([(1.0, d1, d1**2, d1**3),
                               (1.0, rc, rc**2, rc**3),
                               (0.0, 1.0, 2.0 * d1, 3.0 * d1**2),
                               (0.0, 1.0, 2.0 * rc, 3.0 * rc**2)]),
                    npy.array([-e, 0.0, 0.0, 0.0]))
                self.par[Z] = (n, v, rc, N1, N2, d1, d2,
                               A, B, C,
                               E0, E1, E2, E3)
                if rc > self.rc:
                    self.rc = rc
        ...
        self.forces = npy.empty((len(atoms), 3))
        self.densities = npy.empty(len(atoms))
        ...
        for i1 in range(-N1, N1 + 1):
            for i2 in range(-N2, N2 + 1):
                for i3 in range(-N3, N3 + 1):
                    ...
                    c = (i1 == 0 and i2 == 0 and i3 == 0)
                    for a1 in range(natoms):
                        n, v, rc = self.par[self.numbers[a1]][:3]
                        for a2 in range(natoms):
                            if c and a2 == a1:
                                continue
                            d = R[a1] - Q[a2]
                            b = rc - sqrt(npy.dot(d, d))
                            if b > 0.0:
                                b2 = b**2
                                self.energy += v * b * b2
                                self.densities[a2] += n * b2

        for a in range(natoms):
            n, v, rc, N1, N2, d1, d2, A, B, C, E0, E1, E2, E3 = \
               self.par[self.numbers[a1]]
            m = self.densities[a]
            if m < N1 * n * (rc * (1.0 - d1 / d2))**2:
                r = rc - sqrt(m / (N1 * n))
            else:
                r = (-B - sqrt(B**2 - 4.0 * A * (C - m))) / (2.0 * A)

            self.energy -= (N1 * (rc - r)**3 + N2 * (rc - r * d2 / d1)**3) * v
            self.energy += E0 + r * (E1 + r * (E2 + r * E3))
"""
