...
calc.dump_period = 10
calc.run(set_atoms=True)    # set_atoms=True to read final coordinates and velocities after NVE simulation

new_slab = calc.atoms.copy()

Ek = new_slab.get_kinetic_energy()
Ek2 = calc.thermo_content[-1]['ke']
E = calc.thermo_content[-1]['pe']   # do not use  slab.get_potential_energy() because it will run NVE simulation again
